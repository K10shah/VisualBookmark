...
if($count == 1)	
{
	...
	echo "<img id='prof-image' src='".$row['boardpicture']."'>";

echo"<div id='mydiv' ></img></div></div>";
}
echo "<div class = 'edit-profile-submit'><form id='edit-prof-submit' action='editboard.php?boardid=".$row['boardid']."' enctype='multipart/form-data' method='post' name='edit-prof-submit'>
...
if($row['boardcaption']<>NULL)
{
	$descriptionval = $row['boardcaption'];
}
else
	$descriptionval="";


echo " <b><font color='#fff'>Caption: </b><input type='text' value= '$descriptionval' class='form-control col-md-6' id='descr' class='descr' name='descr'>
 <br><br><br>
...
<input type='submit'  class ='btn btn-success' value='Crop' name='ourbtn'>
 ...
 <input type='hidden' id='boardid' value='$boardid' name='pinid'></form> </div>";
 ...
 if(isset($_POST['Upload']))
 {
	 ...
	 $pinid = $_GET['boardid'];
	 $sql = "update boards SET boardpicture='$primg',boardcaption='$desc' where boardid='$pinid'";
	 $result=mysqli_query($con,$sql);
	if(!$result)
		{
			die(mysqli_error($con));
		...
		alert('Successfully updated new content ');</script>";
 }
...
$y1 = $actualwidth/553 * $y1;
$y2 = $actualwidth/553 * $y2;
if ($im !== false) {
    // ob_start();
    // imagepng($im);
...
$contents =  ob_get_contents();
ob_end_clean();
$sql = "update boards SET boardpicture='data:image/jpeg;base64,".base64_encode($contents)."' where boardid='$pinid'";
	 $result=mysqli_query($con,$sql);
	 echo "<script>
	 document.getElementById('descr').value = '$desc';
