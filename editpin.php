...
{
	$row = mysqli_fetch_array($result);
	echo "<div class='pin-image'>";
	if(isset($_POST['linktext']))
	{
		echo "<img id='prof-image' src='".$_POST['linktext']."'>";
	}else
	echo "<img id='prof-image' src='".$row['image']."'>";

echo"<div id='mydiv' ></img></div></div>";
}
echo "<div class = 'edit-profile-submit'><form id='edit-prof-submit' action='editpin.php?pinid=".$row['bookmarkid']."' enctype='multipart/form-data' method='post' name='edit-prof-submit'>
...
<input type='submit'  class ='btn btn-success' value='Crop' name='ourbtn'>
 ...
 <input type='hidden' id='pinid' value='$pinid' name='pinid'></form> </div>";
 ...
 if(isset($_POST['Upload']))
 {
	 ...
	 $pinid = $_GET['pinid'];
	 $sql = "update bookmarks SET image='$primg',caption='$desc' where bookmarkid='$pinid'";
	 $result=mysqli_query($con,$sql);
	if(!$result)
		{
			die(mysqli_error($con));
...
$contents =  ob_get_contents();
ob_end_clean();
$sql = "update bookmarks SET image='data:image/jpeg;base64,".base64_encode($contents)."' where bookmarkid='$pinid'";
	 $result=mysqli_query($con,$sql);
	 echo "<script>
	 document.getElementById('descr').value = '$desc';
